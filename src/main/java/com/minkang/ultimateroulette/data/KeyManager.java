...
public class KeyManager {
    ...
    public ItemStack buildKeyItemLore(ItemStack base, KeyDef def) {
// Build lore from config templates
org.bukkit.inventory.ItemStack copy = base.clone();
...
String lineT = gconf.getString("lore_templates.roulette.line", "&7- &f%NAME%%COUNT% &7(&b%PERCENT%%&7)");

for (String h : header) lore.add(com.minkang.ultimateroulette.util.Text.color(h.replace("%KEY%", def.getName())));

int total = Math.max(1, def.getRewards().stream().mapToInt(Reward::getWeight).sum());
int shown = 0;
for (Reward r : def.getRewards()) {
    org.bukkit.inventory.ItemStack it = r.getItems().isEmpty() ? new org.bukkit.inventory.ItemStack(org.bukkit.Material.CHEST) : r.getItems().get(0);
