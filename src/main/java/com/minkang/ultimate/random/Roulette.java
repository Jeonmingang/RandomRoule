...
import org.bukkit.configuration.serialization.ConfigurationSerializable; import org.bukkit.inventory.ItemStack;
import java.util.*;
public class Roulette implements ConfigurationSerializable {
  ...
  public Roulette(String key){ this.key=key; } public String getKey(){ return key; } public List<RouletteEntry> getEntries(){ return entries; }
  public void setEntries(List<RouletteEntry> e){ entries=e; } public ItemStack getTriggerItem(){ return triggerItem; } public void setTriggerItem(ItemStack i){ triggerItem=i; }
  public boolean isEmpty(){ return entries==null||entries.isEmpty(); } public int getTotalWeight(){ int s=0; for(RouletteEntry e:entries) s+=Math.max(1,e.getWeight()); return Math.max(1,s); }
