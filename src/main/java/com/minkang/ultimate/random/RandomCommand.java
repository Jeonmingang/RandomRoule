...
// Flexible syntax (확장): 
// /랜덤 지급 <키>                                   -> self, amount=1
// /랜덤 지급 <키> <갯수>                            -> self, amount=<갯수>
// /랜덤 지급 <키> <플레이어|player> <닉네임> [갯수]   -> explicit target
// /랜덤 지급 <키> <닉네임> [갯수]                    -> implicit target
if(!isAdmin(sender)){ sender.sendMessage("권한이 없습니다."); return true; }
String key = a.length>=2 ? a[1] : null;
if (key == null) {
    sender.sendMessage("/"+label+" 지급 <키> [플레이어 닉네임|닉네임|갯수] [갯수]");
    return true;
}
