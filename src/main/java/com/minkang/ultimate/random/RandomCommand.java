...
public class RandomCommand implements CommandExecutor, TabCompleter {
    ...
    // V2: Keep original name, put key header & list in lore, and set PDC tag so right-click opens preview
    private static org.bukkit.inventory.ItemStack decorateRouletteKeyV2(com.minkang.ultimate.random.Main plugin, com.minkang.ultimate.random.Roulette r, org.bukkit.inventory.ItemStack base){
        org.bukkit.inventory.ItemStack item = base.clone();
        org.bukkit.inventory.meta.ItemMeta meta = item.getItemMeta();
        if(meta == null) return item;
        // Mark as roulette key using PDC
        if(plugin != null){
            org.bukkit.persistence.PersistentDataContainer pdc = meta.getPersistentDataContainer();
            pdc.set(plugin.getPdcKey(), org.bukkit.persistence.PersistentDataType.STRING, r.getKey());
        }
        // Build lore (do NOT override DisplayName)
        ...
        if(!deco){ item.setItemMeta(meta); return item; }
        ...
        boolean showW = (cfg==null? false : cfg.getBoolean("random.key.show-weight", false));
        ...
        lore.add(0, org.bukkit.ChatColor.translateAlternateColorCodes('&', header.replace("%key%", r.getKey())));
    ...
    // Adds item listing with probabilities into key lore
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] a){
        ...
        if(sub.equalsIgnoreCase("아이템")){
            ...
            ItemStack key = decorateRouletteKeyV2(plugin, r, inHand);
            r.setTriggerItem(key);
            plugin.getManager().save();
            ((Player)sender).getInventory().setItemInMainHand(key);
        ...
        if(sub.equalsIgnoreCase("지급")){
            ...
            Roulette r = plugin.getManager().get(a[1]);
            if(r==null){ sender.sendMessage(plugin.msg("not_found").replace("%key%", a[1])); return true; }
            org.bukkit.OfflinePlayer target = Bukkit.getOfflinePlayer(a[2]);
            int amount;
            try{ amount = Math.max(1, Integer.parseInt(a[3])); }catch(Exception e){ sender.sendMessage("§c개수 숫자"); return true; }
            if(!target.isOnline()){
                sender.sendMessage("§e대상 플레이어가 온라인이 아닙니다.");
                return true;
            }
            org.bukkit.entity.Player tp = target.getPlayer();
            org.bukkit.inventory.ItemStack keyItem = r.getTriggerItem();
            if(keyItem==null || keyItem.getType().isAir()){
                sender.sendMessage("§c해당 룰렛의 키 아이템이 설정되어 있지 않습니다. /랜덤 아이템 <키> 로 먼저 설정하세요.");
                return true;
            }
            for(int i=0;i<amount;i++){
                org.bukkit.inventory.ItemStack give = keyItem.clone();
                give.setAmount(1);
                tp.getInventory().addItem(give);
            }
            sender.sendMessage(plugin.color("&b[랜덤]&7 키 지급 완료: &f" + r.getKey() + " &7x" + amount + " → &a" + tp.getName()));
            return true;
        }
            if(a.length<4){ sender.sendMessage("§c/랜덤 지급 <키> <플레이어> <개수>"); return true; }
            Roulette r = plugin.getManager().get(a[1]);
            ...
            int amount;
            try{ amount = Math.max(1, Integer.parseInt(a[3])); }catch(Exception e){ sender.sendMessage("§c개수 숫자"); return true; }
            if(!target.isOnline()){ sender.sendMessage("§e대상 플레이어가 온라인이 아닙니다. 온라인 접속 시 사용하세요."); return true; }
            org.bukkit.entity.Player tp = target.getPlayer();
            org.bukkit.inventory.ItemStack key = r.getTriggerItem();
