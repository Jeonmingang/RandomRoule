...
public class PreviewGUI implements Listener{
  ...
  public static void open(Main plugin, Player p, Roulette r){
    ...
    org.bukkit.inventory.meta.ItemMeta dm=draw.getItemMeta();
    if(dm!=null){
  ...
  @EventHandler public void onClick(InventoryClickEvent e){
    ...
    if(e.getRawSlot()==49){
      ...
      if(r==null){ ((Player)e.getWhoClicked()).sendMessage(plugin.msg("not_found").replace("%key%", key)); e.getWhoClicked().closeInventory(); return; }
      if(r.isEmpty()){ ((Player)e.getWhoClicked()).sendMessage(plugin.msg("no_items")); e.getWhoClicked().closeInventory(); return; }
      e.getWhoClicked().closeInventory();
      ((Player)e.getWhoClicked()).playSound(e.getWhoClicked().getLocation(), Sound.UI_BUTTON_CLICK, 1f, 1.2f);
      SpinnerGUI.start(plugin,(Player)e.getWhoClicked(), r);
    }
  }
}