...
public class PreviewGUI implements Listener {
  ...
  public PreviewGUI(Main p){ this.plugin=p; }
  ...
  private boolean consumeOneKey(Main plugin, Player p, String key){
    ...
    if(hasKeyTag(plugin, main, key)){
      if(main.getAmount()<=1) inv.setItemInMainHand(new ItemStack(Material.AIR));
    ...
    try{
      ...
      if(hasKeyTag(plugin, off, key)){
        if(off.getAmount()<=1) inv.setItemInOffHand(new ItemStack(Material.AIR));
    ...
    for(int i=0;i<inv.getSize();i++){
      ...
      if(hasKeyTag(plugin, it, key)){
        if(it.getAmount()<=1) inv.setItem(i, new ItemStack(Material.AIR));
        else { it.setAmount(it.getAmount()-1); inv.setItem(i, it); }
        return true;
      }
    }
    return false;
