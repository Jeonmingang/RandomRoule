...
public class SpinnerGUI {
  public static void start(final Main plugin, final Player p, final Roulette r){
    ...
    try{
      glassMat=Material.valueOf(plugin.getConfig().getString("spinner.glass-material","WHITE_STAINED_GLASS_PANE"));
    }catch(Exception ex){ throw new IllegalArgumentException("Invalid glass material in config."); }

    final ItemStack glass=new ItemStack(glassMat);
    ItemMeta gm=glass.getItemMeta();
    if(gm!=null){ gm.setDisplayName(ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString("spinner.glass-name","&f"))); glass.setItemMeta(gm); }
    ...
    final java.util.List<RouletteEntry> entries=new java.util.ArrayList<>(r.getEntries());
    ...
    new org.bukkit.scheduler.BukkitRunnable(){
      ...
      ItemStack withChanceLore(RouletteEntry re){
        ItemStack it=re.getItem().clone();
        ItemMeta m=it.getItemMeta();
        if(m!=null){
          // ★ 스피너에서는 확률 1줄만 표시 (원래 로어/가중치 제거)
      ...
      void paintRow(int center){
        for(int offset=-4; offset<=4; offset++){
          ...
          ItemStack display = chanceOnly ? withChanceLore(entries.get(idx)) : entries.get(idx).getItem();
      ...
      void giveReward(){
        ...
        if(reward.hasItemMeta()){
          ...
          if(m!=null){ m.setLore(null); reward.setItemMeta(m); } // 지급 시 로어 제거
        }
        if(p.isOnline()){
      ...
      public void run(){
        ...
        paintRow(pointer);
        p.playSound(p.getLocation(), Sound.UI_BUTTON_CLICK, 0.5f,
            1.7f - Math.min(1.0f, (float)(totalSteps-stepsLeft)/(float)totalSteps));
