...
if(glow){
        ...
        if(m!=null){
          ...
          // Remove any existing admin-only lines such as weight or chance from the merged lore
          if (lore != null && !lore.isEmpty()) {
            java.util.List<String> cleaned = new java.util.ArrayList<>();
            for (String line : lore) {
              String plain = org.bukkit.ChatColor.stripColor(line == null ? "" : line).trim().toLowerCase();
              String plainNoSpace = plain.replace(" ", "");
              if (plain.startsWith("가중치") || plain.contains("가중치:") || plainNoSpace.contains("가중치:")
                  || plain.startsWith("weight") || plain.contains("weight:")
                  || plain.startsWith("확률") || plain.contains("확률:") || plainNoSpace.contains("확률:")) {
                continue;
              }
              cleaned.add(line);
            }
...
double chance=100.0*re.getWeight()/Math.max(1,totalWeight);
      ...
      void giveReward(){
        ...
        plugin.tryEndSpinOnce(p); // proceed to award regardless of lock state
