...
public class SettingsGUI implements Listener{
  ...
  public static void open(Main plugin, Player p, Roulette r){
    ...
    int i=0; for(RouletteEntry e: r.getEntries()){
      ...
      if(m!=null){
        java.util.List<String> lore=m.hasLore()?m.getLore():new java.util.ArrayList<String>();
        if(lore==null) lore=new java.util.ArrayList<String>();
        boolean replaced=false;
        for(int li=0; li<lore.size(); li++){
          String plain=ChatColor.stripColor(lore.get(li));
          if(plain.startsWith("가중치:")){ lore.set(li, "§7가중치: §e"+e.getWeight()); replaced=true; break; }
        }
  ...
  private void setWeightLore(ItemStack it, int weight){
    ...
    for(int i=0;i<lore.size();i++){
      ...
      if(plain.startsWith("가중치:")){ lore.set(i, "§7가중치: §e"+weight); replaced=true; break; }
    }
