...
public class SettingsGUI implements Listener{
  ...
  public static void open(Main plugin, Player p, Roulette r){
    ...
    int i=0; for(RouletteEntry e: r.getEntries()){
      ...
      if(m!=null){
        ...
        for(int li=0; li<lore.size(); li++){ String plain=ChatColor.stripColor(lore.get(li)); if(plain.startsWith("가중치:")){ lore.set(li, "§7가중치: §e"+e.getWeight()); replaced=true; break; } }
  ...
  private int parseWeight(ItemStack it){
    ...
    for(String line: it.getItemMeta().getLore()){
      ...
      if(s.startsWith("가중치:")){
        ...
        try{ return Math.max(1, Integer.parseInt(num)); }catch(Exception ignore){}
      }
    }
    return 1;
  }
  private void setWeightLore(ItemStack it, int weight){
    ...
    java.util.List<String> lore=m.hasLore()?m.getLore():new java.util.ArrayList<String>(); if(lore==null) lore=new java.util.ArrayList<String>();
    boolean replaced=false;
    for(int i=0;i<lore.size();i++){ String plain=ChatColor.stripColor(lore.get(i)); if(plain.startsWith("가중치:")){ lore.set(i, "§7가중치: §e"+weight); replaced=true; break; } }
  ...
  @EventHandler public void onClose(InventoryCloseEvent e){
    ...
    Roulette r=plugin.getManager().get(key); if(r==null) return;
