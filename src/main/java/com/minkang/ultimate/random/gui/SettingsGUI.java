...
public class SettingsGUI implements Listener{
  ...
  private static org.bukkit.inventory.ItemStack CONTROL_GUIDE(){
    ...
    meta.setDisplayName(org.bukkit.ChatColor.GOLD + "조작 안내");
    java.util.List<String> lore = new java.util.ArrayList<>();
    lore.add(org.bukkit.ChatColor.GRAY + "좌클릭: " + org.bukkit.ChatColor.AQUA + "+1 (가중치)");
    lore.add(org.bukkit.ChatColor.GRAY + "우클릭: " + org.bukkit.ChatColor.AQUA + "-1");
    lore.add(org.bukkit.ChatColor.GRAY + "Shift + 좌/우클릭: " + org.bukkit.ChatColor.AQUA + "±10");
    lore.add(org.bukkit.ChatColor.DARK_GRAY + "※ 설정 화면에서만 보이는 안내이며 보상 아이템에는 영향 없음");
    meta.setLore(lore);
    paper.setItemMeta(meta);
    return paper;
  }

  private static void showProbabilityInSettings(org.bukkit.inventory.ItemStack it, com.minkang.ultimate.random.Roulette r, com.minkang.ultimate.random.RouletteEntry e){
    try{
      org.bukkit.inventory.meta.ItemMeta m = it.getItemMeta();
      if(m==null) return;
      java.util.List<String> lore = m.hasLore()? new java.util.ArrayList<>(m.getLore()) : new java.util.ArrayList<>();
      boolean showW = ((com.minkang.ultimate.random.Main)org.bukkit.Bukkit.getPluginManager().getPlugin("UltimateRandomRoulette")).getConfig().getBoolean("settings.show-weight", true);
      boolean showC = ((com.minkang.ultimate.random.Main)org.bukkit.Bukkit.getPluginManager().getPlugin("UltimateRandomRoulette")).getConfig().getBoolean("settings.show-chance", true);
      int total = Math.max(1, r.getTotalWeight());
      int w = Math.max(1, e.getWeight());
      if(showW) lore.add(org.bukkit.ChatColor.GRAY + "가중치: " + org.bukkit.ChatColor.GOLD + w);
      if(showC){
        double prob = 100.0 * w / total;
        lore.add(org.bukkit.ChatColor.GRAY + "확률: " + org.bukkit.ChatColor.AQUA + new java.text.DecimalFormat("#.##").format(prob) + "%");
      }
      m.setLore(lore);
      it.setItemMeta(m);
  ...
  public static void open(Main plugin, Player p, Roulette r){
    ...
    int i=0; for(RouletteEntry e: r.getEntries()){
      ...
      ItemMeta m=it.getItemMeta();
            showProbabilityInSettings(it, r, e);
      if(m!=null){
    ...
    try{ inv.setItem(Math.min(inv.getSize()-1, 53), CONTROL_GUIDE()); }catch(Throwable ignored){}
    try{ inv.setItem(Math.min(inv.getSize()-1, 53), CONTROL_GUIDE()); }catch(Throwable ignored){}
    p.openInventory(inv);
  }
